...
class SimpleModel(nn.Module):
    def __init__(self, latent_size: int, input_shape: Tuple[int, int, int] = (60, 512, 512)):
        ...
        self.encoder = nn.Sequential(
            nn.Conv3d(1, 16, kernel_size=4, stride=2, padding=self.padding),  # Assuming single-channel input
            nn.ReLU(),
            nn.Conv3d(16, 32, kernel_size=4, stride=2, padding=self.padding),
            nn.ReLU(),
            nn.Conv3d(32, 64, kernel_size=4, stride=2, padding=self.padding),
            nn.ReLU(),
            nn.Flatten()
        )

        self.reshape = (64, (self.depth // 8), (self.height // 8), (self.width // 8))
        ...
        self.decoder = nn.Sequential(
            nn.ConvTranspose3d(64, 32, kernel_size=4, stride=2, padding=self.padding),
            nn.ReLU(),
            nn.ConvTranspose3d(32, 16, kernel_size=4, stride=2, padding=self.padding),
            nn.ReLU(),
            nn.ConvTranspose3d(16, 1, kernel_size=4, stride=2, padding=self.padding),
            nn.Sigmoid()
        )
...
if __name__ == '__main__':
    ...
    model = SimpleModel(input_shape=input_shape, latent_size=latent_size)
